...
/**
 * This class represents a pointed-top Hexagon tile used to make up a
 * <code>Maze</code> object.
 * <p>
 * Each tile has a type. It can be a Wall, Start, Enqueued, Dequeued,
 * backtracker, and a current hex.
 * <p>
 * Each Hex has a stepsToMe value which tracks how many steps it took to get
 * from the start to this hex. The default value is -1.
 * <p>
...
public class Hexagon extends HexComponent {
	...
	public static enum HexType {
		WALL, START, END, UNVISITED, ENQUEUED, END_PROCESSED, START_PROCESSED, DEQUEUED, BACKTRACK, CURRENT
	};
	...
	private int r, c; // grid row and col to determine approx euclidian distance
	 ...
	 */
	public Hexagon(HexType t, int r, int c) {
		this.type = t;
	 ...
	 *            inclusive)
	 * @throws InvalidNeighborIndexException
	 ...
	 * 
	 * The index for the neighbor indicates which side of the hexagon the neighbor
	 * to get is on. 0-5 inclusive.
	 * 
	 * @param i
	 *            The index indicating the side of the hexagon this neighbor is on
	 * @return The hexagon the is on the i-th side of the current hexagon, or null
	 *         if no neighbor
	...
	/**
	 * Returns the predicted distance from this hex to the end of the maze. The
	 * predicted distance is simply the Euclidean distance. If there is no end to
	 * the maze then a distance of 0 is returned
	 * 
	 * 
	 * @param The
	 *            maze the hexagon is from
	 * @return The predicted distance to the end hex.
	 */
	public double distanceToEnd(Maze m) {
		try {
			return Math.sqrt(Math.pow(((double) this.r - (double) m.getEnd().r), 2)
					+ Math.pow(((double) this.c - (double) m.getEnd().c), 2));
		} catch (NullPointerException e) {
			return 0.0;
		}
	}

	public void printPriority(Maze m) {
		double d = distanceToEnd(m);
		System.out.println(stepsToMe + "+" + d + "=" + (d + (double) stepsToMe));
	}
	 ...
	 * 
	 * @return true if this was in the queue but has since been dequeued, false
	 *         otherwise.
	 */
	...
	/**
	 * This method sets the tile to be a enqueued tile and updates the tile's colour
	 */
	...
	/**
	 * This method sets the tile to be a processed finish tile and updates the
	 * tile's colour
	 */
	...
	/**
	 * This method sets the tile to be a processed start tile and updates the tile's
	 * colour
	 */
	...
	/**
	 * This method sets the tile to be the current hex being looked at and updates
	 * the tile's colour
	 */
	...
	/**
	 * This method sets the tile to be in the backtracking path and updates the
	 * tile's colour
	 */
	 ...
	 * 
	 * @param t
	 *            The type to use to set the color
	 */
